...
**Parallel next implementation block：V7c — Linux system service（dedicated low-privilege account + machine state）**

V6b-3a的secret-free sign/verify pipeline、schema-3、exact frozen-layout gate、Windows/macOS无凭据fail-closed、native independent verify与unsigned non-regression均已收口；V6b-3b只剩外部正式签名凭据。V7a `systemd --user` 与V7b Windows machine Connector service/user-session control plane均已封板。V7b-2 implementation commit `3ebc9fe` 已在cap00完成restricted-token安全验收，并在mzd Active RDP Session 2完成可见GUI+close-to-tray native acceptance。现在进入V7c：Linux machine scope必须使用专用低权限service identity与独立machine state，默认Connector-only，不复用user DeviceKey；install/enable/start/stop/disable/uninstall仍保持显式分离。Distribution Studio signed-output/cache继续后置到真实签名acceptance。
