...
| V8+ | Demand-driven expansion | IN PROGRESS | V8.1 Agent filesystem completeness + desktop UX 已完成 Windows 本地实现，等待用户验收；其余 federation/relay/第二 transport 继续后置 |
...
- `SiteKitContract` 冻结同 runtime / 同 `site.clew` 的双入口 argv：普通入口 `host`，helper 入口 `host --connector-only`，Windows/macOS/Linux 一致。**这里在 V1.5 阶段冻结的是产品语义 contract；V6c-3 已按该 contract 落地 Windows/macOS/Linux 双 role launcher 与 Site Kit assembly。正式 Windows/macOS release-ready 签名仍由 V6b-3b 外部凭据 gate 管理。**
...
V1.5 final multi-machine acceptance 已推进到真实三机产品路径；2026-09-03 的连续物理诊断又把 blocker 从“LAN 拓扑”与“Windows GPO/Defender”两种过早归因中进一步收窄：**当前测试网络对普通终端之间的 peer UDP/QUIC direct ingress 存在上游 ACL / isolation，无法作为稳定的 physical Connector acceptance 拓扑**。该轮只把 physical Site Kit 双 launcher/signing 延后到 V6；V6c-3 后续已完成双 launcher/assembly，Windows/macOS 正式签名 acceptance 仍只受 V6b-3b 外部凭据 gate 阻塞。
...
V5 single-file双向 data plane至此封板。当前 resume仍只保证同进程的 InnerSession generation变化；Controller/Host进程重启后的 task/part/source state继续按 V5a-3 分块收口。`Feature::FileResume` 在 Controller/Host 两端 restart gate全部通过前继续保持reserved/not-implemented。

### V5a-3a — Controller durable transfer journal foundation

**Status：DONE（2026-09-04）**

- Controller file-transfer manager新增独立双槽 generation journal `controller-file-transfers.{a,b}.json`，仍使用全局 state envelope/document hard bound；journal最多 **16 transfers**，TransferId重复、progress字段不一致、非canonical SHA等在load阶段fail closed；
- journal显式绑定当前 `ControllerId`。旧/异Controller identity的transfer state不能被新Controller静默接管；wrong Controller focused直接 `ControllerMismatch`。如果newest slot损坏，仍可回退到上一份完整有效generation；两份同generation冲突直接拒绝；
- Controller startup改为`ControllerFileTransferManager::load_or_create(...)`。active Put在Controller重启后恢复为`WaitingForReconnect`并重建worker；它不信任journal里的旧confirmed offset，而是在Host重新在线后用既有Status/manifest规则对齐真实part checkpoint，因此无需每4KiB重写整份Controller journal；
- cancel状态在发cancel signal前先journal化；task终态再persist。durable manager存在时Drop不再把transfer当“CLI/session owned task”统一取消，符合Controller长期ownership；非durable test manager继续保持旧Drop cleanup；
...
- exact-clean closeout固定在implementation/fix commit `21fcee81b8c9bceecd8f61e25692c9aaca1ba7d0`。Windows/macOS native closeout已完成；Linux wmn02 exact checkout重新独立复核：source release ZIP SHA=`dae668baf3bed80010dc45a37c6f66d69f3baacc080b0e6a5c32cc1745bc509a`，cache key=`client-flavor-v1-28ea3aa57d0e0f8ef618e4b4f69505bab7caa8164f72939a9fc8e533138f505e`且`release_ready=true`；最终 `V6C3 Linux Site-Clew-Linux.tar.gz` SHA=`898cb443bb5270643d99ac123552894edd27edcd350eafc670e279383ec751c1`。独立 `verify-package` PASS；展开后8个受管文件 exact set/size/SHA/mode 全匹配sidecar，normal/helper marker分别为`use-this-machine`/`connector-only`，主runtime与两个role launcher均为native x86-64 ELF，主runtime `--version`=`clew 0.1.0`。V6c-3封板。
...
- Windows dirty packaging smoke 已确认新 payload 为 `Clew Launcher.exe, README.md, clew.exe`，PE header 反读 `launcher_subsystem=2`（Windows GUI）而后台 runtime `subsystem=3`（Console/CLI）。该检查已下沉为 `xtask verify-package` 的硬门禁：Windows human entrypoint 必须 subsystem=2，CLI/runtime 必须 subsystem=3；launcher schema 在过渡混合产物上也被 strict verifier 正确拒绝，证明 v1/v2 隔离生效。
- implementation 固定为 `d82dd6f5e76188f92b644eb50cd8958e6b038cff`。从 clean HEAD 本地重建的 Windows `0.2.0` release `dirty=false / entrypoint=Clew Launcher.exe / launcher schema=2`，ZIP SHA=`7e31bac894643a092dfc745baa14813727075a4bf00fc1515caafb9c5befe630`，launcher/runtime SHA 分别=`8d2a3c7effa9dbb1d1889b86844889f9fb2a95b9ddd504ba3947c8c2239cdff8` / `a2999ddf329b8989bece10cfb544710e5688017870b920e9a3d3448ade77478d`，PE subsystem=`2/3`，独立 `verify-package` PASS。由同一 artifact 进入 rehearsal cache `client-flavor-v1-cb7f725f136405fdc663c38621ea11ace29071370cc933ae1d24ce3b62f1c10c` 后，用该 exact `0.2.0` runtime 启动 Controller 并真实签出 `site.clew`（SHA=`f19faea1548233be7994206b6a8a523cd3f0038adae3a4b5eb85e41580e0ebb2`），组装 Site Kit ZIP SHA=`a31bc7910a04bec7a49787927e74915b105b0be6de21b4f819462091133872a2`；独立解包只有 6 个受管文件，恰好一个根 `Clew.exe` 与一个 `.clew-runtime/clew.exe`，旧双 role 目录/`role-hint.clew` 数量为 0，且两个 EXE SHA 精确等于源 release launcher/runtime。一次刻意暴露的 stale `target/release/clew.exe 0.1.0` negative control 被 ClientFlavor mismatch 正确拒绝，没有绕过版本绑定。
- public CI run `33958800912` 在 exact `d82dd6f5...` 全绿：Windows 2022、macOS 14、Linux manylinux2014/glibc-2.17 的 test/package/upload 全部 PASS。annotated tag `v0.2.0` peeled 到同一 commit；release workflow run `33959436013` 全绿，Windows/macOS/Linux 三份 unsigned release-input 均重新构建并验真，`Sign Windows`、`Sign and notarize macOS`、signed publish 对 0.x 正确 skipped，`Publish unsigned GitHub Release` 成功。`v0.2.0` 为 normal Release（非 draft / 非 prerelease），7 assets 全 uploaded：macOS ZIP SHA=`ee382dfe50b3aaf8da23431d0eafa81866880c0cdaf49d173db203782dac3c33`，Windows ZIP SHA=`a88f25a778e63e381cb941dbf0a06e29990aaadad3d8c5105c32bba22552e441`，Linux ZIP SHA=`be12232cc74a0a2cf8a2aeb7586e8ecced742660e455054fb0122d417afe0f0b`，release-level `SHA256SUMS` asset SHA=`2ebcfc6ee86c48a0a9f3d86acd42f102ba8c6befca6f7583475b2baa132f3f30`；从 GitHub 重新下载 Windows ZIP 后当前 `xtask verify-package` 再次 PASS。V6d 封板。
...
**V7b-1 Windows machine Connector service：DONE。** 实现 commit `f38af72346db4fab78cbb7a2d53452c4779ee46e` 新增 `--scope machine` 的 install/enable/start/stop/disable/uninstall/status、SCM dispatcher、ProgramData machine layout、LocalService +独立 service SID、protected 3-ACE DACL、binary/Site Kit hash binding 与有限 failure-restart。machine runtime强制 `HostLaunchMode::ConnectorOnly`，因此同一 execute-preferred Site Kit 在 service scope 只会得到 `execute=false / connector=true`；portable/user DeviceKey不迁移。start/enable要求payload完整，stop/disable/uninstall仅要求Clew SCM identity + metadata，从而损坏安装仍可清理。Windows Local API named-pipe endpoint同时修复等价state-root拼写问题，`target\x` / `.\target\x` / absolute path现在共享endpoint。Windows service/ACL focused **4/4**、controller CLI **10/10**、workspace **263/0/6**，all-target check 0 warning。

cap00 exact-commit native acceptance（`f38af723...`）：ControllerId保持 `a39b564f-b7f3-89fd-b238-a8b6704af662`，Controller用`target\...`启动而status/devices用`.\target\...`均成功；install 后 `manual/stopped`。Windows独立反读 machine root `AreAccessRulesProtected=true` 且只有 SYSTEM / Administrators / `S-1-5-80-2310617593-914737494-356934921-3458674384-1746943570` 三条非继承 FullControl OI+CI ACE；`qsidtype=UNRESTRICTED`，SCM account=`NT AUTHORITY\LocalService`，failure actions为2s→10s→60s restart、3600s reset，binary/Site Kit hash与metadata一致。enable 后仍 stopped；start 后新 machine DeviceId=`f3f44d9d-89ce-4163-ac1c-88befee361b1` 为online + connector-only。stop 后同ID offline且SCM pid=0；restart后同ID恢复。强杀 service pid `105492` 后SCM自动拉起 `107124`，同ID再次online。停止后篡改ProgramData binary，start因hash不匹配fail closed，但disable/uninstall仍成功；最终SCM entry、ProgramData root、隔离Controller均清空。V7b-1封板。
...
## 16.1 V8.1 — Agent filesystem completeness + desktop UX acceptance

**Status：IMPLEMENTATION COMPLETE / ELEVENTH USER ACCEPTANCE PASSED / v0.2.1 RELEASED（2026-09-06）**

用户完成 `v0.2.0` 后第一轮真实桌面验收，明确触发此前 deferred 的 V8+：现有协议/数据面已经具备成熟的 resumable file/directory transfer，但 MCP 尚未暴露；GUI 虽尝试 light visuals，实际窗口 clear/theme 仍可呈现黑底；Site Kit 文件系统范围只能填写单个 root；Write/Edit 之外缺乏面向 agent 的安全文件系统生命周期操作。V8.1 作为一个连续产品块推进：

1. **Desktop visual system**：Controller / Host / Windows role launcher 显式锁定 Light theme 与 light clear color，统一浅色页面、白色 card、单一蓝色 primary、弱化技术元数据；主界面按 Basic/Advanced 分层，`Create Site Kit` 本身可折叠；A/B/C 角色恒定为 A=Controller、B=Target、C=Helper。第二轮桌面反馈后，Controller 增加 Site Kit → collaborator devices → Agent MCP 三阶段矢量状态轨、Target/Helper 设备图标和连接状态点，Host 增加 B/C 角色状态图，Launcher 增加连接拓扑 mark；Controller 默认窗口增至 1180×840、Host 780×600、Launcher 720×560，三者主体均可滚动，小屏/缩放不再截断主流程。fresh Controller 时 Create Site Kit 默认展开，devices/MCP 仅在已有对应状态时成为默认展开项。第四轮重新收紧操作归属：`Create Site Kit to send...` / `Cancel` 回到 `1. Create Site Kit` 表单内部；固定 footer 只保留全局 `Hide to tray` / `Exit Clew` 与 Controller / Runtime / Site Kit / MCP 四栏状态。布局先硬保留 118px footer + 6px gap，再把所有剩余高度交给主 ScrollArea，不再以最小滚动高度反向挤掉状态栏。
2. **Filesystem scope**：Signed `ReadPolicy` 增加显式 `all_filesystem` 模式，GUI 默认选择“B 上所有文件夹（仍受 B 当前 OS 账户权限约束）”，同时保留 restricted roots；不能用 `*` 等伪 root 绕过 canonical containment。Write/File Put 等仍受独立 signed grant 门禁。
3. **MCP transfer completeness**：直接复用 V5 durable data plane，把单文件 Put/Get/Status/Cancel 与目录 Put/Get/Status/Cancel 暴露为 typed MCP tools；Controller-local path 与 B-side path 在 schema/description 中明确区分，不另造传输协议。第二轮增加 `transfer_put_batch` / `transfer_get_batch` / `transfer_batch_status` / `transfer_batch_cancel`：每次 start 最多 8 个根项，单项通过 `recursive` 选择既有 single-file plane 或既有 bounded recursive directory plane；batch 只聚合真实 durable child TransferId，不创建易丢失的 transient BatchId。status/cancel 一次最多 32 个引用。所有 transfer tool 支持 `minimal` / `summary` / `full` verbosity；现有单项工具默认 `full` 并保持原完整 JSON shape，batch 默认 `summary`。
4. **Controlled filesystem operations**：禁止把 destructive delete 退化为 Shell `rm`。新增受 signed Write authority + path policy + replay protection 约束的 first-class filesystem control：mkdir/copy/move；删除先进入 OS Trash/Recycle Bin 并记录 Clew operation metadata；永久删除必须经过 prepare/confirm 两阶段短期确认 token，不能一次调用直接 purge。
5. **Managed temporary resources**：B 上临时文件/目录只能通过 Clew managed namespace 创建，记录 stable resource id、kind、description/purpose、created/expiry；提供 list/release/gc，避免 agent 在任意目录留下无法追踪的孤儿。GC 只处理 Clew 自己登记的资源。
6. **Acceptance-state isolation + local runtime bootstrap**：第一轮 local package 的根 Launcher 在无 `site.clew` 时执行 `clew gui` 且未传 `--state-dir`，因此复用了 `%LOCALAPPDATA%\\Clew`，造成旧 Collaborator computers 混入。r2 改成 package-local `.clew-local-acceptance-state` 后又遇到 ClientFlavor cache 与 Windows 深路径问题；r4-r6 因此改成短 `%TEMP%\\clew-a-<16hex>`，但该 key 绑定 exact extraction root + manifest，随后真实 r6 连接验收证明每个 rN 会静默轮换 Controller state/ControllerId，破坏 local acceptance 的升级连续性。r7 起 dirty Launcher 固定使用独立且短的 `%LOCALAPPDATA%\\Clew-Acceptance-v1`，但 r7 实测又暴露迁移缺口：r6 已登记 mzd/t8plus 的 Controller `99ea...` 留在 `%TEMP%\\clew-a-*`，r7 在稳定 namespace 新建 Controller `5a0d...` 且 catalog devices=0，因此 A 与 B/C 实际属于不同 Controller。r8 只在 `dirty=true` local acceptance 下扫描旧 `clew-a-*`：有 enrollment 的 legacy state 若 busy 则本次直接复用，idle 则 staged copy、二次解析比对 ControllerId/device count/generation，保存原 r7 空 state 为 superseded archive 后原子迁入稳定 namespace；正式 `dirty=false` Release 完全不进入该迁移路径。正式 verifier 不放宽：`ControllerConfig::local_acceptance_runtime` 默认 false，acceptance store 仅允许 unsigned/no-signing/0.x dirty runtime 且永远 `release_ready=false`。Runtime import 失败仍有独立持久 UI error。
7. **MCP permission/limit/path contract alignment**：第四轮用户确认 r5 GUI 改善且 MCP file transfer 可用，但 `clew_read(device=mzd,path=~/.ssh/config,limit=65536)` 被误报为 `Denied: read is not permitted`。根因不是 `all_filesystem` 丢失，而是 GUI 签名/transport 的 Read hard result budget 是 48 KiB（49152），MCP 却把 65536 原样透传，Controller 又把“超过 signed max”与“没有 Read authority”合并成 Denied。r6 保留 48 KiB transport headroom，不扩大 wire risk：MCP 的 read/glob/grep maximum hints 安全 cap 到 hard bounds，Shell timeout/attach、managed temp TTL 同样归一化；file/directory custom chunk size 在 MCP 层先校验 power-of-two + protocol range。Controller 将真正 authority/revocation/inactive device 维持 `Denied`，signed/hard budget 超限明确归类 `InvalidRequest`，并把 read/query/mutation 的 active-device gate 与 file/shell/tcp 对齐。Target B 的用户路径统一支持绝对路径或当前 Clew OS 账户的 `~/...`（Windows 同时支持 `~\\...`），`~otheruser` 拒绝；展开后仍走原 canonicalization、OS ACL、signed root/all-filesystem、symlink/reparse 防护。该统一层覆盖 Read/PathInfo/Glob/Grep、Write/Edit、file/directory Put/Get、controlled FS 与 Shell cwd；helper C 的执行权限边界不变。
8. **Live connection truth + mDNS fail-open discovery**：r6 真实 A/B/C 验收出现 A 显示 disconnected、B/C 长期显示 Connected 但实际无 live session。直接原因之一是 Host GUI 把持久 `HostLaunchState::Active`（仅表示 enrollment/membership 已保存）错误等同于在线。r7 新增 `HostNetworkState::{Offline,Connecting,Connected,Reconnecting,Unavailable}` watch channel；只有 InnerSession 双向认证完成后 B 才能 Connected，C 还必须收到并验证 Controller-signed Connector lease 后才 Connected；stream/heartbeat/lease 路径掉线后立即进入 Reconnecting。更深的连接问题是 active/bootstrap 路径把 `MdnsConnectorDiscovery::attach(...)` 用 `?` 当硬门禁：Windows mDNS 初始化失败会连 direct/nearby-file 一起放弃，甚至会在 C 已认证且拿到 lease 后主动拆掉 C→A。r7 把 mDNS 恢复为 optional LAN convenience：attach 失败只失去自动组播发现，不杀 direct session、不阻断 imported nearby file，并且 C 仍保存 signed `nearby-connection.clew` export。r8 再把 Connected 真值收紧为双端一致：A 先 `RemoteHub.register()` 当前 DeviceId/generation，再复用既有 `session_ping/session_pong` 立即证明 registration；B/C 只有收到该 proof（或旧 Controller 在 register 后发出的首个合法 RPC）才发布 Connected，首个 RPC 会保留并继续正常执行。A 的 authenticated RemoteHub/heartbeat 仍是最终在线 authority。
9. **Local acceptance runtime-cache freshness**：r8 已恢复连接后，用户再次用 MCP 读取 `~/.ssh/config`，绝对路径 `C:/Users/inter/.ssh/config` 成功但 shorthand 被旧 Host 报 `outside allowed roots`。现场二进制/provenance 取证证明当前 B 实际仍运行 r5 runtime：mzd Site Kit `source_release_sha256=abf5e2f9...84c14`、runtime SHA=`f7db46c6...d1176`，而 r8 runtime SHA=`77454f2e...ab294e`；A 的 `%LOCALAPPDATA%\\Clew-Acceptance-v1` active ClientFlavor 同样仍指向 r5。根因是 r8 legacy Controller-state migration 误把 package-derived `v1/client-flavors` cache 一起迁入，GUI 看到同 flavor/version active 后跳过当前 package import；同时 dirty rN 的 cache key 只绑定 flavor/version/source_commit/signing，没有绑定实际 payload file hashes，因此同为 `0.2.0` 的本地包会碰撞。r9 三层修复：acceptance migration 明确不复制 `v1/client-flavors`；dirty-only ClientFlavor cache identity 额外绑定完整 release payload digest（clean/signed release 维持原 v1 key 兼容，旧 dirty cache 仍可只读验证）；local-acceptance GUI 每次启动即使已有 active flavor 也必须验证/导入当前 package 一次，且 refresh 失败时 Site Kit create fail-closed，不能继续打包 stale runtime。
10. **Agent-friendly Read + explicit Site Access Credentials + strict A/B/C validation**：r9 真实验收确认当前 runtime 已能直接读取 `~/.ssh/config`，但 MCP 始终返回 `data_base64`，在无 `atob` / Node `Buffer` 的 agent 环境非常不自然；同时 Controller 主视图把同 hostname 的历史 enrollment 与当前 B/C 平铺，用户无法知道哪套 Site Kit/credential 正在使用。r10 将 `clew_read.output` 设为 `auto|text|base64`：默认 `auto` 对合法 UTF-8 直接返回 `encoding=utf-8 + text`，二进制/非法 UTF-8 自动退回 base64，显式 `text` 对非法 UTF-8 fail closed。每次 Create Site Kit 仍创建全新 `InviteId`，但现在把它作为一等 **Site Access Credential** 管理：稳定非 secret 短 ID `CRED-<InviteId 前 48-bit>`；A 可选择历史 credential，主 Collaborator computers 只投影所选 credential 的 enrollment，因此旧 mzd/t8plus 不再和当前项混排；A GUI 显示 InviteId、完整 pass fingerprint、claims/expiry/signed permissions，并提供 Close（仅禁止新 enrollment）、Revoke（二次确认，撤销并断开其设备）、Delete old credential（二次确认，从普通历史和设备投影删除，但保留最小 revoked tombstone/fingerprint，旧 Site Kit 永远不能因“删除记录”而复活）。同一 `CRED-...` + 完整 InviteId 同时显示在 A 创建完成提示、A credential panel、Site Kit 根 Launcher、B/C Host GUI、`Start Here.html` 与 `Message to collaborator.txt`，两端可人工核对但 bootstrap bearer secret 永不展示。另新增 B-only `--require-helper` / Launcher checkbox `Private-network validation: require helper C for target B`：显式启用时 bootstrap 和 active InnerSession 都禁止 direct B→A，只接受当前 Site 的 verified Connector candidate；C 仍按 normal connector-only 直接连 A，因此即使 B 本身有公网，也能严格证明 A↔C↔B 路径。默认 Automatic 行为保持 direct/Connector race，不改变普通用户体验。第十轮独立复核进一步收紧 credential lifecycle 根层语义：`EnrollmentRegistry::close_invite/revoke_invite` 不再用 `entry(...).or_default()` 接受未知 InviteId，而是统一返回 `UnknownInvite`；Local API 的 Close/Revoke/Delete 对 unknown credential 因而全部 fail closed，且不会制造不可见的 phantom credential/state。
11. **Helper reachability self-heal + multicast-independent LAN discovery + Helper visibility**：r11 真机复测出现 C=t8plus 已连 A、B=mzd 勾选 helper 后始终离线。现场取证确认两层真实环境问题：其一，t8plus 活动网卡是 Windows Public profile、默认 Inbound Block；旧 Clew Allow 规则只绑定此前 WIP runtime 路径，当前 `Desktop\\CD\\...\\.clew-runtime\\clew.exe` 没有适用规则，而 Iroh QUIC 使用动态 UDP 端口；其二，即使补了当前 runtime 的 UDP LocalSubnet allow，删除 B 的 nearby hint 后 mDNS 在该 `/22` 网络仍无法发现 C。r13 因此把 helper 可达性做成产品能力而非验收手工步骤：Windows C 启动前按 **current executable path** 检查 `Inbound Allow / UDP / LocalPort Any / RemoteAddress LocalSubnet / Profile Any`，不存在时通过 hidden `windows-helper-firewall-ensure` → UAC `RunAs` → self-scoped installer 建规则并再次反查，拒绝 UAC/策略失败则 fail closed；规则名含当前 runtime path hash，不按固定 QUIC 端口放行。发现面保留 persisted signed Nearby file + mDNS，同时新增同 Site 有界 UDP LAN probe：由 `SiteDiscoveryTag` 派生稳定发现端口、128-bit nonce、4 KiB datagram/16 candidate/900 ms 界限，Target 对本地 directed/limited broadcast 探测，C 只在已完成 C→A 认证并拿到 Controller-signed Connector lease 后响应，且只接受本机 IPv4 subnet 来源；probe 只给 `EndpointAddr` candidate，真实 tunnel 仍必须通过原 `ConnectorReady` signed lease 验证，`--require-helper` 仍绝不启动 direct B→A。Helper C GUI 同步增加 privacy-preserving tunnel telemetry：显示 active Target session、enrollment tunnel 与 launch-since total 计数；C 不解密 inner session，也不伪称知道 B hostname/文件/命令，因此“具体 Target 名字”继续保留到未来显式 metadata + privacy review，而不是破坏当前 E2E 边界。

cap00 第十一轮 helper-reachability review 已把用户 r11 现场失败从“Helper 已连但 Target 不见”收敛为两层可复现环境事实并在 exact r13 上闭环：当前 Site Kit 路径没有适用 Windows Firewall UDP allow，且同一 `/22` 的 mzd/t8plus 在 unicast 可达时仍无法依赖 mDNS；手工 signed nearby hint + 临时 firewall 曾作为诊断对照成功并由 A 证明 `topology=connector`。生产修复现改为 current-runtime firewall self-heal + UAC、multicast-independent same-Site LAN discovery，并在 C GUI 增加不窥视 inner plaintext 的 Target session 计数。当前 frozen gate：`cargo fmt -- --check` PASS；`cargo check --workspace --all-targets --locked` 0 warnings；`cargo test --workspace --all-targets --locked` **322 passed / 0 failed / 8 ignored**；Host **71/0/4 ignored**。focused LAN same-site/nonce discovery **2/2**、Helper tunnel lifecycle **1/1**、Windows firewall path-scoped rule identity **1/1**、strict `require_helper_mode_uses_connector_even_when_controller_direct_is_reachable` PASS。fresh r13=`dist/v8.1-local-r13/clew-v0.2.0-x86_64-pc-windows-msvc.zip`，SHA256=`9b1db6be3a80b1786ea6287a812d6d9dad1b9990530932b083fd1a564fb82f40`；independent verifier `verified=true/schema2/unsigned`，fresh extraction embedded manifest `dirty=true/source_commit=d2b33092cca60e6d1934c355f4816641628ebc99/launcher schema=2`，runtime SHA=`d8f49f79848c15c27dec893d435c42a4eeb14da71dc924912ea007beaf288650`。t8plus 真机 elevated installer + normal ensure 已创建并反查 `Clew Nearby Helper 494c73d6eed2`：exact current program / UDP / Any dynamic port / LocalSubnet / Profile Any；交互式 UAC 点击路径因 SSH 非交互环境留给桌面验收。最终 exact-r13 三机 gate 在 mzd 明确 `KIT_HINT=False / IMPORT_COUNT=0` 下完成：C 与 B 同时 Online，B DeviceId `d5092ba0-433d-4674-82d0-ac7876376ae6` 的 A telemetry 为 generation 13 / `topology=connector`，同一 session 真实 `read ~/.ssh/config` 成功。该证据不依赖 manual nearby file，也不允许 direct B→A。用户已完成 r13 真实桌面验收并明确确认连接恢复；该实现已进入公开 `v0.2.1`，V8.1 的本轮 acceptance/release 门禁关闭。

## 17. 当前 checkpoint

**Current signed-major release gate：V6b-3b — Real credential acceptance（BLOCKED by external credentials）**

**Parallel unblocked implementation block：none — V8.1 is released as v0.2.1; signed-major credential gate remains external**

V7 Advanced Service Runtime、V6d zero-terminal desktop distribution UX 与 V8.1 Agent filesystem/desktop UX 均已封板并公开到 `v0.2.1`。r6 filesystem permission/path contract、r7 mDNS fail-open、r8 Controller identity continuity + bilateral online truth、r9 exact runtime-cache freshness、r10 explicit Site Access Credential + UTF-8 Read + strict ConnectorRequired、r11 unknown-credential fail-closed，以及 r13 Windows helper firewall self-heal + multicast-independent bounded same-Site LAN discovery 均进入当前公开基线。普通生产连接仍 Automatic，正式 clean/signed cache 和 `%LOCALAPPDATA%\\Clew` 路径保持不变；Windows safe Trash、all-filesystem、durable transfer、managed temp 与 MCP 48 KiB Read contract 均保持。当前公开 normal Release 为 GitHub `v0.2.1`，exact release commit=`8bd94330da7697bd604043dce3da1eeac848b18e`；main CI `34010829451` 与 release workflow `34010839242` 均全绿。signed-major blocker 仍只有 V6b-3b 外部 Windows/macOS 凭据 acceptance；wmn02 n0 relay egress 仍是环境问题。

### Change log

- **2026-09-06** — `v0.2.1` patch release **DONE**：用户明确认可 r13/V8.1；功能实现先冻结为 commit `1e5409d`。workspace version 已从 `0.2.0` 提升到 `0.2.1`，Cargo.lock 只更新 Clew 自身 9 个 workspace package version、无第三方依赖漂移，README current release 同步更新。release-candidate gate：`cargo fmt -- --check` PASS；`cargo check --workspace --all-targets --locked` PASS；`cargo test --workspace --all-targets --locked` **322 passed / 0 failed / 8 ignored**。发布仍沿用既有 0.x normal unsigned policy，Windows/macOS 不冒充签名，Linux 继续 manylinux2014/glibc-2.17 gate。release commit=`8bd94330da7697bd604043dce3da1eeac848b18e`，annotated `v0.2.1` 精确指向该 commit；tag-triggered release run `34010839242` 全绿，main CI `34010829451` 亦全绿。正式 GitHub Release=`Clew 0.2.1`（normal / 非 draft / 非 prerelease），7 assets 全 uploaded；macOS ZIP SHA=`155cefe73939092620c5d144cee98ba9de2cf54afd0e3f1e8083d1f5886f7d90`，Windows ZIP SHA=`fadaf6b1bcd4c220c75bd7ab5b94e815073ee6bad7c2d5f8485bbba641052b5f`，Linux ZIP SHA=`2f3bf4944ad61f7db0847315d1a3e8f4d9dd1037498ad51a6f305c52cdc5067a`，release-level `SHA256SUMS` asset SHA=`2b3c59ba8d8fa56600aaa5270c3cd183c0f2525a6d0d16756c0fa8eacccc4958`。从 GitHub 重新下载 Windows ZIP + manifest 后，当前 `xtask verify-package` 再次 PASS（`verified=true/schema2/unsigned`，`source_commit=8bd94330...`、`dirty=false`）。Release notes 已补充 V8.1 的 UTF-8 Read、完整 MCP file plane、Site Access Credential、准确连接状态、Helper firewall/UAC + LAN discovery 与 E2E privacy 说明。GitHub Actions 仅留下 Node.js 20 action-runtime deprecation annotation（当前被 runner 强制 Node 24，未影响本次 gate）；后续维护 action major 时处理。

- **2026-09-06** — V8.1 eleventh local acceptance helper reachability + automatic LAN fallback **IMPLEMENTATION COMPLETE / USER ACCEPTED / RELEASED IN v0.2.1**：用户在 r11 真机看到 t8plus(C) 已连接 A，但 mzd(B) 勾选 helper 后无法上线。现场确认 C 当前 runtime 在 Windows Public profile 下没有适用 inbound rule；旧 Allow rules 只绑定历史 WIP path，而 C 当前 Iroh 监听 UDP 62463/62464。补临时 exact-program UDP/LocalSubnet rule 后 manual signed nearby hint 可让 B 上线且 A 报 `topology=connector`，但删除 hint/import 后即使 unicast 网络正常，mDNS 仍长期发现不到 C，因此 firewall 与 multicast discovery 是两层独立根因。r13 新增 current-runtime firewall self-heal/UAC（应用级 UDP Any dynamic port + LocalSubnet，拒绝提权 fail closed）、same-Site bounded UDP LAN discovery fallback（仍以 Controller-signed Connector lease 为最终 authority），并给 Helper GUI增加 active/bootstrap/total Target tunnel count而不解密 inner plaintext。真机 installer 首轮还抓出当前 `netsh` 不接受 `group=`，r13 修正后 t8plus production rule `Clew Nearby Helper 494c73d6eed2` install+ensure+ActiveStore 反查 PASS。exact-r13 在 mzd `KIT_HINT=False / IMPORT_COUNT=0` 下 C/B 同时 Online，B generation 13=`topology=connector`，真实 `read ~/.ssh/config` PASS。locked workspace **322/0/8 ignored**、Host **71/0/4 ignored**、check 0 warning、fmt PASS；fresh r13 ZIP SHA=`9b1db6be3a80b1786ea6287a812d6d9dad1b9990530932b083fd1a564fb82f40`，independent verifier PASS。随后用户完成真实桌面流程并确认现在可以正常使用；V8.1 验收门禁关闭，并已随 `v0.2.1` 正式发布。

- **2026-09-06** — V8.1 tenth local acceptance credential-lifecycle consistency review **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：独立复核 r10 后确认 MCP `auto|text|base64`、按 InviteId/CRED 分组设备、deleted credential tombstone 与 bootstrap+active 双阶段 `ConnectorRequired` 均无绕过；另发现 identity registry 的 `close_invite/revoke_invite` 对未知 InviteId 使用 `entry(...).or_default()` 可制造不可见 phantom state。现改为 unknown `InviteId` 统一 `UnknownInvite`，Local API Close/Revoke/Delete 均返回 InvalidRequest，且 registry 不发生污染；新增 identity + Local API 两层回归。frozen locked workspace **318/0/8 ignored**，check 0 warning，fmt PASS；focused credential/Connector/MCP Read 全绿。fresh r11 ZIP=`c6f2dadedc43bdaa212870d92e8f10e84ed051bd1bd18953d1b5883936be0f6f`，independent verifier `verified=true/schema2/unsigned`，fresh extraction embedded manifest `dirty=true/source_commit=d2b33092.../launcher schema2`。r10 三机 physical gate 继续作为 strict Connector 功能证据，但 r11 未冒充 exact-artifact 三机 gate；用户桌面验收基线切换为 r11。

- **2026-09-06** — V8.1 ninth local acceptance explicit credential + UTF-8 Read + strict Connector-path sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户确认 r9 已能正常连接并直接读取 `~/.ssh/config`，但 MCP 返回 raw base64；同时 A 的 Collaborator computers 中旧/新 mzd、t8plus 各两套平铺，且此前隐式 credential/state 迁移让故障排查非常困难。实现 `clew_read.output=auto|text|base64`，auto 默认 UTF-8 text、binary fallback base64；把每个 Site Kit 的 InviteId 显式建模为 Site Access Credential，A GUI 按 CRED 选择后只显示该 credential 的 B/C，展示 fingerprint/InviteId/claims/expiry/permissions，并支持 Close/Revoke/Delete old credential；Delete 清理 normal history/devices 但持久保留 revoked tombstone，旧 Kit 不可复活。统一非 secret `CRED-<InviteId 前 48-bit>` 显示在 A、Site Kit Launcher、B/C Host、Start Here 与 collaborator message。A/B/C 新增 B-only `--require-helper`/Launcher checkbox，显式模式同时关闭 bootstrap 与 active direct B→A；即便 A endpoint 可达的 focused test 仍通过 Connector。frozen source：locked workspace **316/0/8 ignored**、Host **68/0/4**、Runtime **71/71**、host CLI **3/0/1**、fmt/locked check/diff-check PASS。fresh r10 package SHA=`cdeb2272...a919` / verifier PASS；cap00(A)+mzd(B)+t8plus(C) physical Connector-required gate 已补证：C direct、B connector，且 B `~/.ssh/config` real Read 成功；全部 smoke state/process 已清理。

- **2026-09-06** — V8.1 eighth local acceptance exact-runtime freshness sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户确认 r8 连接已恢复，但 MCP `clew_read(~/.ssh/config)` 仍被 B 报 `outside allowed roots`，而 `C:/Users/inter/.ssh/config` 成功。检查当前 r8 Host 源码确认 expand→canonicalize→policy 顺序正确；mzd 环境 `USERPROFILE=C:\\Users\\inter` 也正确。随后 hash/provenance 取证坐实实际混版本：mzd Host SHA=`f7db46c6...d1176`，Site Kit `source_release_sha256=abf5e2f9...84c14`（r5）；A stable acceptance active ClientFlavor 也同样是 r5，而 r8 runtime SHA=`77454f2e...ab294e`。根因是 r8 Controller-state migration 复制了 `v1/client-flavors`，GUI 因已有 active same-flavor 跳过 current package import，加上 dirty 0.2.0 cache key 未绑定 payload hashes，导致旧 runtime 长期 masquerade 为当前 runtime。r9：migration 排除 ClientFlavor cache；dirty cache key 额外绑定完整 payload digest且保留 legacy dirty read compatibility；local-acceptance GUI 每次启动强制验证/导入当前 package一次，失败时 Site Kit create fail-closed。locked workspace **309/0/8 ignored**，distribution **13/13**，fmt/check PASS。fresh r9 SHA=`af9ccc618b99ccf2bbbb6c8a3d9d3de02720d9be26e96aba4428ff681f640e99`，runtime SHA=`37dc2b7d0d8c5d89a127c7d33b9b0acd1d1a16e57c4e99092b1286f07ada1f65`，verifier PASS。isolated real-r5-cache smoke：active `73c1...` 自动切到新 `999d1f...`，artifact/runtime SHA 分别精确等于 r9 ZIP/runtime；旧 r5 entry 保留但 inactive，smoke 已清理且未动用户真实 state。

- **2026-09-06** — V8.1 seventh local acceptance identity continuity + bilateral online truth sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户反馈 r7 仍然“A 未连接、B/C 显示连接”。现场取证坐实 r7 稳定 acceptance state 是全新 Controller `5a0d...` 且 devices=0，而 r6 `clew-a-7faf...` Controller `99ea...` 仍登记 mzd(B)+t8plus(C)，所以根因首先是 A identity 被 r7 切断；其次 Host r7 Connected 仍只证明自身 InnerSession/lease，早于 A RemoteHub registration。r8 Launcher 仅对 dirty local acceptance 增加 legacy state continuity：有设备的旧 state 若 busy 则直接复用，idle 则 staging copy、二次验证 ControllerId/device count/generation、保存 r7 空 state archive 后原子迁入稳定 namespace；formal release 不扫描/迁移。session 真值改为 A `RemoteHub.register()` 后立即复用 `session_ping/session_pong` 做 generation ready proof，Host 收到 proof 或旧 A 的首个合法 RPC 后才 Connected，并保留首个 RPC。单测：launcher 4/4、runtime 70/70、host 67/0/4 ignored；locked workspace 307/0/8 ignored。真实 mzd smoke 使用 r6 state 副本 + 原 membership：A online false→true、真实 bounded Read 成功、断 Host 后 true→false；smoke 已全部清理。t8plus SSH 本轮超时，不将未执行的 C 并发测试冒充通过。fresh r8=`dist/v8.1-local-r8/clew-v0.2.0-x86_64-pc-windows-msvc.zip`，SHA256=`bc359b268c2c8e14b8233bc864193c94eea6a0b0623c58098a8cd7050cbdb34f`；independent verifier PASS（`verified=true` / schema 2 / unsigned），fresh extraction=`dist/v8.1-local-r8/acceptance/clew-v0.2.0-x86_64-pc-windows-msvc`，内嵌 manifest `dirty=true`。

- **2026-09-06** — V8.1 sixth local acceptance connection truth/recovery sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：r6 真实 A/B/C 出现 A offline、B/C 假 Connected。取证确认 23:49 实际 Site Kit 的 ControllerId 与 A state 一致，且此前 r6 activity 曾成功完成 mzd file_get/path_info/file_put，说明是后续 live session 丢失而非从未 enrollment。修复两类生产问题：其一，Host GUI 原把 `HostLaunchState::Active` 直接映射 Connected，现以独立 `HostNetworkState` 跟踪 Connecting/Connected/Reconnecting/Unavailable/Offline，B 需完成 InnerSession，C 还需验证 signed Connector lease；断流后真实进入 Reconnecting。其二，bootstrap/active/C presence 原把 mDNS attach 当硬门禁，Windows multicast 初始化失败会放弃 direct/nearby-file，甚至拆掉已认证 C→A；现 mDNS fail-open 为 optional discovery，C 仍保持 live session 并保存 signed nearby export。另修 dirty acceptance state：r4-r6 的 extraction+manifest hash 会每版轮换 ControllerId，r7 改固定 `%LOCALAPPDATA%\\Clew-Acceptance-v1`，与 production state 隔离但跨 rN 稳定。frozen source：locked workspace **307/0/8 ignored**、Host **67/0/4 ignored**、root/CLI **39/0/2 ignored**、locked check/fmt/diff-check PASS；NearbyFile no-mDNS 与 connector-only focused PASS；public n0 relay explicit gate **1/1 PASS**。fresh r7=`dist/v8.1-local-r7/clew-v0.2.0-x86_64-pc-windows-msvc.zip`，SHA256=`14a306e58b93f6123711b094ac2f5760d7096ca3440d38aac7c1f6700ba34e6f`；independent verifier PASS（schema 2 / unsigned），fresh extraction=`dist/v8.1-local-r7/acceptance/clew-v0.2.0-x86_64-pc-windows-msvc`，manifest `dirty=true`，且验收前确认 `%LOCALAPPDATA%\\Clew-Acceptance-v1` 仍不存在，未被自动 smoke 污染。

- **2026-09-06** — V8.1 fifth local acceptance permission/limits sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户确认 r5 GUI 改善且 MCP file transfer 可用，但 all-filesystem Kit 的 `clew_read(~/.ssh/config, limit=65536)` 被 Controller 误报 Denied。定位为 MCP 64 KiB hint > signed/transport 48 KiB result budget，不是 all-filesystem/read grant 丢失。r6 不扩大 48 KiB wire bound，而在 MCP 对 read/glob/grep maximum、Shell budgets、temp TTL 做安全 cap，对 transfer chunk 做 early validation；Controller 将 authority/revoked/inactive 维持 Denied，把 protocol/signed budget 错误改为 InvalidRequest，并补齐 read/query/mutation active-device gate。新增 Target current-account `~/...` 统一展开，覆盖 Read/query、Write/Edit、file/directory Put/Get、controlled FS、Shell cwd，`~otheruser` 拒绝且 canonical/ACL/signed scope 仍生效。MCP schema 同步说明该路径语义。frozen locked workspace **305/0/7 ignored**，MCP **8/8**，Host **66/0/4 ignored**，Runtime **70/70**，fmt/check/diff-check PASS。fresh r6 SHA=`ad56237a...e08d9`，independent verifier PASS，GUI dirty-runtime bootstrap PASS。需用 r6 重新生成 Kit 后复测原命令。

- **2026-09-05** — V8.1 fourth local acceptance layout correction **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户在 r4 真实 GUI 明确指出底部职责仍然反了：Site Kit 的 Create/Cancel 不应固定在全局 footer，而 Hide/Exit 才应常驻，同时状态栏实际被裁掉。已将 Create/Cancel + readiness blocker 放回 `1. Create Site Kit` 表单内；滚动区末尾旧 Hide/Exit 删除；固定 footer 第一行只保留全局 `Hide to tray` / `Exit Clew`，第二行使用四等分列持续显示 Controller/Runtime/Site Kit/MCP。布局取消会把 footer 推出窗口的 `scroll_height.max(260)`，改为先保留 118px footer + 6px gap，再将剩余高度全部分配给 ScrollArea。源码结构检查确认 Create/Hide/footer-height 各只有一个目标位置；locked workspace **301/0/7 ignored**、check PASS。fresh r5 ZIP=`abf5e2f9...84c14`，independent verifier PASS，fresh extraction `dirty=true` 且无 package-local acceptance state；r5 short-state GUI bootstrap 再次得到 `release_ready=false / dirty=true` 并完成清理。现交用户第四轮真实桌面验收。

- **2026-09-05** — V8.1 third local acceptance sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户在 r2 真实 GUI 看到 `Create Site Kit to send...` 灰掉，并要求底部状态栏与两个 action 常驻。根因链已完整定位：fresh isolated state 没有 ClientFlavor cache；production verifier 正确拒绝 dirty runtime；此前 package-local acceptance state 还把 ClientFlavor staging 深路径推到约 319 chars，Windows 无法 final publish。最终实现显式 `local_acceptance_runtime` policy，默认/正式路径仍 strict；仅 dirty Launcher 允许 unsigned/no-signing/0.x dirty runtime 且始终 `release_ready=false`。Acceptance state 改为 canonical extraction+manifest hash 派生的短 `%TEMP%\\clew-a-<16hex>`；r4 实际 57 chars。Controller footer 从主 ScrollArea 分离，Create/Cancel 常驻，第二行状态栏持续显示 Controller/Runtime/Site Kit/MCP 与 blocker；runtime import 错误单独保持可见。locked workspace **301/0/7 ignored**，check PASS；r4 SHA=`221a1a75...870e2` / verifier PASS；Launcher-equivalent r4 smoke 在 **10.679 s** 得到 active runtime，`release_ready=false / dirty=true / controller_ready=true`，测试 state 已清理。

- **2026-09-05** — V8.1 second local acceptance sweep **IMPLEMENTATION COMPLETE / USER ACCEPTANCE PENDING**：用户反馈第一轮窗口内容被裁切、fresh flow 被旧 Collaborator 状态干扰，并要求 transfer recursive/batch/verbosity 与更直观的图形状态。已确认旧设备来自 root Launcher 正常复用 `%LOCALAPPDATA%\\Clew`；dirty local manifest 现在触发 package-local isolated state，而 formal release 不变。Controller/Host/Launcher 扩大 viewport 并加入 vertical ScrollArea；Controller 增加 3-step status rail 与 Target/Helper glyph，Host/Launcher 增加角色/拓扑 glyph。MCP 新增四个 bounded batch tools，最多 8 个 start roots / 32 个 status-cancel refs，recursive 项直接复用 V5 directory plane，全部返回真实 durable TransferId；minimal/summary/full 三档输出且 full 保持旧结构。focused MCP **6/6**、launcher isolation **1/1**、locked workspace **299/0/7 ignored**、check/diff-check PASS。fresh r2 ZIP=`410d02e8...38bdd`，independent verifier PASS；解包后 `dirty=true / state_exists_before_first_launch=false`，PE subsystem=`2/3`、runtime stack reserve=8 MiB。现交用户第二轮体验。

- **2026-09-05** — V8.1 Windows local implementation **COMPLETE / USER ACCEPTANCE PENDING**：桌面视觉系统重构为显式浅灰页面 + 白色 cards + 单一蓝色 primary，Create Site Kit 默认折叠；signed `ReadPolicy::all_filesystem` 成为 GUI 默认且 restricted root 仍可选；MCP 增加 file/directory Put/Get/Status/Cancel 与 typed controlled FS surface（mkdir/copy/move/trash/restore/purge prepare+confirm/temp lifecycle）。Windows 删除后不依赖 Session-specific Shell Recycle Bin enumeration，而是精确解析同卷 `$Recycle.Bin` `$I/$R` metadata，真实 cap00 Session 0 acceptance 完成 restore 与两阶段 purge；managed temp 带 resource id/description/TTL/ABOUT.txt，GC 只处理 Clew-owned namespace。大型 Clap command tree 在 Windows 默认主线程栈暴露 stack overflow 后，build.rs 固定至少 8 MiB stack reserve，xtask verifier 新增 PE stack reserve gate；`controller_cli` 12/12 恢复。frozen local：fmt/check/diff-check PASS，workspace **297/0/7 ignored**，Windows Recycle Bin explicit gate **1/1**，local package SHA=`87fd42da...8695e` 且新 verifier PASS。macOS exact Trash restore/purge 仍 fail-closed Unsupported，未冒充跨平台闭环。

- **2026-09-05** — V6d zero-terminal UX + `v0.2.0` **DONE**：implementation=`d82dd6f5e76188f92b644eb50cd8958e6b038cff`。Windows 人类 entrypoint 分离为 GUI-subsystem `Clew Launcher.exe` 与后台 CLI/runtime `clew.exe`；Site Kit 根单 `Clew.exe` GUI 选择 target/helper，所有后台 child hidden console；Controller GUI 直接收集权限、验证 extracted Release、生成完整 Site Kit并管理 loopback MCP。launcher contract 升 v2，legacy v1 只验真不组新包；unsigned 0.x 可用但 1.x+ fail closed。frozen workspace locked run **293 passed / 0 failed / 6 ignored**，fmt/check/diff-check PASS。exact-clean Windows release ZIP=`7e31bac8...e630` 与真实 Site Kit ZIP=`a31bc791...72a2`闭环通过，Site Kit 为 6-file 单 launcher 布局且源 launcher/runtime SHA 精确保持、PE subsystem=`2/3`。public CI `33958800912` 三平台全绿；annotated `v0.2.0` 精确指向 implementation commit，release run `33959436013` 三平台 input + normal unsigned publish 全绿，signed jobs 对 0.x 正确 skipped。最终 GitHub Release 7 assets 全 uploaded；Windows/macOS/Linux ZIP 分别=`a88f25a7...e441` / `ee382dfe...3c33` / `be12232c...0f0b`，GitHub 下载后的 Windows asset 再跑当前 verifier PASS。
...
- **2026-09-05** — V6c productization **DONE / V6c-3/4b/5 closed**：V6c-3 Linux exact checkout `21fcee8` 的 release ZIP=`dae668ba...509a`、release-ready cache与最终 Site Kit tar=`898cb443...51c1`重新独立验真，8-file exact hash/mode + 双marker + native ELF均PASS；Controller GUI现以matching active native ClientFlavor readiness为硬门禁，默认复合`SiteKitCreate`输出完整Site Kit，sidecar-only降到Advanced；readiness错配回归PASS。README/GUI/Connector/Distribution Studio/gaps全部消除旧“未来V6 launcher”陈述。workspace locked check 0 warning，workspace **285/0**。当前只剩V6b-3b外部Windows/macOS签名凭据gate。
