...
**Parallel unblocked implementation block：none — V6d zero-terminal desktop distribution UX is closed**

V7 Advanced Service Runtime 与 V6d zero-terminal desktop distribution UX 均已封板。Windows `0.2.x` 产品形态已从“双 role 目录 + CLI/GUI console 尾窗”收口为 Release 根 GUI-subsystem `Clew Launcher.exe`、Site Kit 根单一角色选择 `Clew.exe`、后台 `clew.exe` hidden console，以及 Controller GUI 内建分发向导与 MCP lifecycle；旧 launcher v1 仅保留历史验真兼容。仓库公开于 `https://github.com/m2selfA/clew`；public CI 继续以 Windows 2022、Linux glibc 2.17 与 macOS 14 为 gate。当前公开 normal Release 已更新为 `v0.2.0`（`https://github.com/m2selfA/clew/releases/tag/v0.2.0`），其自动 release workflow 已完整成功。发布策略保持：Windows/macOS unsigned `0.x` 可普通发布但必须明确 unsigned，未来 Code Signing + Developer ID/notary 齐备后升新的 `1.x+` major 走 signed gate。当前唯一 signed-major blocker 仍是 V6b-3b 外部凭据 acceptance；除该 gate 外没有已知未完成的 V6/V7 实现块，V8+保持 deferred。wmn02 n0 relay egress 仍是环境问题。
